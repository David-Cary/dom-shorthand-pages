 ...
 */
export class LocalNameValidator {
  nameAttribute: string
  readonly depthSearch = new DepthFirstSearch()
  readonly vertexFactory = new ValueVertexFactory([
    getValidContentNodeVertex
  ])
  ...
  getMatches (source: ValueMap[]): LocalNameMatches {
    ...
    const contextStack: NamedContentMatchNode[] = []
    this.depthSearch.startPhasedTraversal(
      source,
      (state) => {
        ...
        if (
          ...
          let match: NamedContentMatchNode | undefined
          ...
          if (id != null) {
            ...
            match = {
              route: cloneRoute(state.route),
              nameMatches: {}
            }
            matches.idMatches[id].push(match)
          }
          const name = getAttributeValue(target, this.nameAttribute, 'localName')
          if (name != null) {
            if (match == null) {
              match = {
                route: cloneRoute(state.route),
            ...
            if (contextStack.length > 0) {
              ...
              if (context.nameMatches[name] == null) {
                context.nameMatches[name] = []
              }
              context.nameMatches[name].push(match)
            } else {
              if (matches.rootMatches[name] == null) {
                matches.rootMatches[name] = []
              }
              matches.rootMatches[name].push(match)
            }
          }
          if (match != null) {
      ...
      (state) => {
        const context = contextStack[0]
        if (context?.route.target === state.route.target) {
          contextStack.shift()
        }
      },
      this.vertexFactory
   ...
   */
  getUniqueNamesIn (
    source: Record<string, NamedContentMatchNode[]>,
    duplicates?: AmbiguousContentSearchResult[],
    searchProperty = 'localName',
    path: Array<ValueMap | ValidKey> = []
  ): Record<string, NamedContentTraversalNode> {
    const uniques: Record<string, NamedContentTraversalNode> = {}
    for (const key in source) {
      ...
      if (matches.length === 1) {
        ...
        uniques[key] = {
          route: match.route,
          children: this.getUniqueNamesIn(
            match.nameMatches,
            duplicates,
            'localName',
            path
        ...
        duplicates.push({
          search: path.slice(),
          results: matches.map(match => match.route)
        })
      }
      path.pop()
    }
  ...
  validate (source: ValueMap[]): LocalNameValidation {
    const matches = this.getMatches(source)
    const results = this.validateMatches(matches)
    return results
  }
  ...
  validateMatches (matches: LocalNameMatches): LocalNameValidation {
    const duplicates: AmbiguousContentSearchResult[] = []
    const results: LocalNameValidation = {
      target: matches.source,
      idValues: this.getUniqueNamesIn(
        matches.idMatches,
        duplicates,
        'id'
      ),
      rootValues: this.getUniqueNamesIn(
        matches.rootMatches,
        duplicates
      ),
      duplicates
 ...
 */
export class ValidatedNameSearchResolver {
  readonly subsearchResolver: IdentifiedContentSearchResolver

  constructor (nameAttribute = 'data-local-name') {
    this.subsearchResolver = new IdentifiedContentSearchResolver(nameAttribute)
  }
   ...
   */
  resolve (
    context: LocalNameValidation,
    path: Array<ValueMap | ValidKey>,
    maxResults?: number
  ): SearchResponse {
    const nameSearch = this.findNamedNode(context, path)
    if (nameSearch.route != null) {
      const response: SearchResponse = {
        state: {
          route: cloneRoute(nameSearch.route),
          visited: [],
          completed: false
        },
        results: []
      ...
      if (nameSearch.subpath.length > 0) {
        this.subsearchResolver.extendSearch(
          response,
          nameSearch.subpath,
          maxResults
        )
    ...
    const delegated = this.subsearchResolver.resolve(context.target, path, maxResults)
    return delegated
  }
  ...
  findNamedNode (
    context: LocalNameValidation,
    path: Array<ValueMap | ValidKey>
  ): PartialSearchResolution {
    if (path.length > 0) {
      const firstStep = path[0]
      if (typeof firstStep === 'object') {
        if ('id' in firstStep) {
          const id = String(firstStep.id)
          const identifiedItem = context.idValues[id]
          if (identifiedItem != null) {
            ...
            const response = this.findNamedChildNode(
              identifiedItem,
              subpath
            )
            return response
          ...
          const name = String(firstStep.localName)
          const namedItem = context.rootValues[name]
          if (namedItem != null) {
            const subpath = path.slice(1)
            if (subpath.length < 1) {
              return {
   ...
   */
  findNamedChildNode (
    context: NamedContentTraversalNode,
    path: Array<ValueMap | ValidKey>
  ): PartialSearchResolution {
    if (path.length > 0) {
      const firstStep = path[0]
      if (typeof firstStep === 'object') {
        if ('localName' in firstStep) {
          const name = String(firstStep.localName)
          const child = context.children[name]
          if (child != null) {
            const subpath = path.slice(1)
            if (subpath.length < 1) {
              return {
